...
- ALT (Alternate Allele): In a VCF variant record, `REF` and `ALT` are standard fields—`REF` is the reference allele and `ALT` lists one or more alternate alleles observed at that position. In this repository, `ALT` is the main variant-level target in `high` mode.
